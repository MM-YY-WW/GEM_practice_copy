...
from pahelix.utils.compund_tools import mol_to_geognn_graph_data_MMFF3d
...
class DownstreamTransformFn(object):
    ...
    def __call__(self, raw_data):
        """
        Gen features according to raw data and return a signle graph data.
        Args:
...
class DownstreamCollateFn(object):
    ...
    def __Call__(self, data_list):
        ...
        Args:
            ...
            create node geatures and edge features according to pgl graph, and then
            use graph wrapper to feed join graph, then the label can be arrayed to batch label.
        Returns:
            The batch data contains finetune label and vaild, which are
            collected from batch_label and batch_vaild.
        """
        ...
        for data in data_list:
            ab_g = pgl.Graph(
                ...
                edge_feat={name: data[name].reshape([-1, 1]) for name in self.bond_names + self.bond_angle_float_names}
