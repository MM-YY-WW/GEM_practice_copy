...
import time
import six
#this could use to deal with the imported version confliction, good point!
if six.PY3:
...
def get_downstream_task_names(dataset_name, data_path):
    ...
    if dataset_name  == 'bace':
        ...
        task_name = get_default_toxcast_task_names()
    elif dataset_name == 'esol':
        task_name = get_default_esol_task_names()
    elif dataset_name == 'freesolv':
        task_name = get_default_freesolv_task_names()
    elif dataset_name == 'lipophilicity':
        task_name = get_default_lipophilicity_task_names()
    elif dataset_name == 'qm7':
        task_name = get_default_qm7_task_names()
    elif dataset_name == 'qm8':
        task_name = get_default_qm8_task_names()
    elif dataset_name == 'qm9':
        task_name = get_default_qm9_task_names()
    elif dataset_name == 'qm9_gdb':
        task_name = get_default_qm9_gdb_task_names()
    else:
...
def calc_rocauc_score(labels, preds, valid):
    ...
    if len(rocauc_list) == 0:
        raise RuntimeError("No positively labeled data available. cannot comput ROC-AUC")
    return sum(rocauc_list)/len(rocauc_list)


def calc_rms(labels, preds):
    """
    tbd
    """
...
def exempt_parameters(src_list, ref_list):
    """
    Remove element from src_lsit that is in ref_list
    """
    res = []
    for x in src_list:
        flag = True
