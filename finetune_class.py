...
def train(args, model, train_dataset, collate_fn, criterion, encoder_opt, head_opt):
    ...
    list_loss = []
    model.train()
    for atom_bond_graphs, bond_angle_grpahs, valids, labels in data_gen:
...
def main(args):
    ...
    model_config:
    a json file with the hyperparameters, such as fropout rate, learning rate, num tasks and so on
    num_tasks:
    ...
    if not args.dropout_rate is None:
        compound_encoder_config['dropou_rate'] = args.dropout_rate
    ...
    encoder_opt = paddle.optimizer.Adam(args.encoder_lr, parameters=head_params)
    ...
    print('Test pos/nege ratio %s/%s' % get_pos_neg_ratio(test_dataset))
    ...
    for epoch_id in range(args.max_epoch):
        ...
        print("epoch: %s test/auc: %s" % (epoch_id, val_auc))
        print("epoch: %s test/auc: %s" % (epoch_id, test_auc_by_eval))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--task", choive=['train', 'data'], default='train')
    parser.add_argument("--batch_size", type=int, default=2)
    parser.add_argument("--num_workers", type=int, default=2)
    parser.add_argument("--max_epoch", type=int, default=100)
    parser.add_argument("--dataset_name", choices=['bace','bbbp','clintox', 'hiv', 'muv', 'sider', 'tox21', 'toxcast'])
    parser.add_argument("--data_path", type=str, default=None)
    parser.add_argument("--cached_data_path", type=str, default=None)
    parser.add_argument("--split_type", choices = ['random', 'scaffold', 'random_scaffold', 'index'])
